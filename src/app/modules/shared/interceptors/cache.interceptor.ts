...
export class CacheInterceptor implements HttpInterceptor {
    ...
    intercept(
        ...
        this.timerService.startTimer(); // each request start new time + 30 minute
        let remainingTimeInMilliseconds = this.timerService.getRemainingTime();

        if (remainingTimeInMilliseconds <= 0) { // reset cache if remainingTime is almost exceeded and next request it should start new timer
